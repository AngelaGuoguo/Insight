...
def frequency_counts(con,path_in,path_out):
    ...
    # request icu events
    icu = load_sql(con,"SUBJECT_ID,INTIME,OUTTIME,LOS","mimiciii.ICUSTAYS")
...
def readmission_diff(con,patients):
    ...
    readmission = pd.merge(first_icu,second_icu,on='subject_id',how='inner'\
                       ).sort_values(by=['subject_id'])

    readmission['diff']=(readmission['intime']-readmission['outtime']).dt.days
...
def extract_adults(con):
    ...
    patients['combined_dod'] = patients.apply(lambda row: \
                 dod(row['dod'], row['dod_hosp'],row['dod_ssn']), axis=1)
    ...
    icu_patients['age']=(icu_patients[u'intime']-icu_patients[u'dob']).dt.days/365
...
def lived(patients):
    ...
    combined = pd.concat([still_lived,discharged]).sort_values(by=\
               ['subject_id']).drop(['dod','combined_dod','outtime'],axis=1)
...
def generate_heart_patients(con,patients):
    ...
    merged = pd.merge(comorbidity,hadms,on=['subject_id','hadm_id'],how='inner')
    return merged
...
def main():
    ...
    adults.to_csv("lists/adults_admitted.csv",index=False,columns=\
                  ['subject_id','combined_dod','outtime','age'])              
    # ----
    ...
    adults_heart = pd.merge(adults,heart_patients,on=['subject_id'],how='inner')
    ...
    generate_icu_id(con,"lists/readmission_diff.csv","lists/first_icu_list.txt")
  
    # print "now run: "
    # print "awk -F, 'FNR==NR{k[$1]=1;next;} FNR==1 || k[$4]' filter_list file.csv"
