...
def retrieve_notes_sql():
    ...
    icu_list = load("notes/adults_heart_discharged.csv",cols=['subject_id','hadm_id'])
...
# start reference: http://stevenloria.com/finding-important-words-in-a-document-using-tf-idf/
def tf(word, doc):
...
def tokenize_notes(text):
    ...
    stopped = [i for i in tokens if not i in stop]
...
def main():
    ...
    # generate top words
    generate_top_words('notes/tokenized_all.txt','notes/top_words_all_notes.txt')
