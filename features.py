...
def generate_phys_scores(con, icu_list, p_list):
    ...
    oasis_last = load_sql(con, "subject_id,icustay_id, oasis",
                          "mimiciii.oasis_last")
    oasis_last = oasis_last[oasis_last['icustay_id'].isin(icu_list)].\
        sort_values(by=['subject_id'])

    saps = load_sql(con, 'subject_id,icustay_id, saps', 'mimiciii.saps')
    saps = saps[saps['icustay_id'].isin(icu_list)].\
        sort_values(by=['subject_id'])

    sapsii = load_sql(con, 'subject_id,icustay_id, sapsii', 'mimiciii.sapsii')
    sapsii = sapsii[sapsii['icustay_id'].isin(icu_list)].\
        sort_values(by=['subject_id'])

    sapsii_last = load_sql(con, 'subject_id,icustay_id, sapsii',
    ...
    icu_los = icu_los[icu_los['icustay_id'].isin(icu_list)].\
        sort_values(by=['subject_id'])
